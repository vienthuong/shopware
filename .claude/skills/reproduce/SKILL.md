...
repo). The **unattended CI path** runs as a GitHub Agentic Workflow (`gh aw`) twin at
`.github/workflows/reproduce.md` and emits JSON via `upload-artifact`. Both surfaces
share one rubric so they cannot drift.
